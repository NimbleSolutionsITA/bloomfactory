...
function windowLoadInit() {
	...
	if (jQuery().slider) {
		...
		if ($rangeSlider.length) {
			...
			var $minR = (urlParams.get('min_price') > "") ? urlParams.get('min_price') : 0;
            var $maxR = (urlParams.get('max_price') > "") ? urlParams.get('max_price') : 200;
			$rangeSlider.slider({
				range: true,
				min: 0,
				max: 200,
            ...
            $('input[name="min_price"]').val($rangeSlider.slider("values", 0));
            $('input[name="max_price"]').val($rangeSlider.slider("values", 1));
		}
	}
